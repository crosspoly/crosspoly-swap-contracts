...
contract CPStaking is Context, Ownable, ReentrancyGuard {
    ...
    constructor(
        ...
        require(_stakingToken != address(0), "CPStaking: ZERO_STAKING_ADDRESS");
        require(_cpToken != address(0), "CPStaking: ZERO_CPTOKEN_ADDRESS");
        require(_startClaimBlockIndex >= _startStakingBlockIndex, "CPStaking: INVALID_START_CLAIM_BLOCK_INDEX");
    ...
    function stake(uint amount) external nonReentrant _logs_ {
        require(!_stakingPaused, "CPStaking: STAKING_PAUSED");
        require(amount > 0, "CPStaking: INVALID_STAKING_AMOUNT");
    ...
    function unstake(uint amount) external nonReentrant _logs_ {
        require(!_withdarawPaused, "CPStaking: UNSTAKE_PAUSED");
        require(block.number >= startUnstakeBlockIndex, "CPStaking: UNSTAKE_NOT_STARTED");

        StakingRecord storage rec = _stakingRecords[_msgSender()];

        require(amount > 0, "CPStaking: ZERO_UNSTAKE_AMOUNT");
        require(amount <= rec.staked, "CPStaking: UNSTAKE_AMOUNT_EXCEEDED");
    ...
    function claimProfit() external nonReentrant _logs_ {
        require(!_claimProfitPaused, "CPStaking: CLAIM_PROFIT_PAUSED");
        require(block.number >= startClaimBlockIndex, "CPStaking: CLAIM_NOT_STARTED");

        uint totalProfit = _getTotalProfit(_msgSender());
        require(totalProfit > 0, "CPStaking: ZERO_PROFIT");
    ...
    function collect(address token, address to) external nonReentrant onlyOwner _logs_ {
        require(token != StakingToken, "CPStaking: COLLECT_NOT_ALLOWED");
    ...
    function _pushShareToken(address to, uint amount) internal {
        ICP(CPToken).mintLockedToken(to, amount);
    }
